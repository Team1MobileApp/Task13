...
public class RouteAction extends Action {
    ...
    public String perform(HttpServletRequest request) {
        ...
        if (origin == null) origin = "5000 forbes ave, pittsburgh, pa";
        if (dest == null) dest = "10 second ave, pittsburgh, pa";
        ...
        try {
            ...
            for (CandidateRoute r : routes)
            {
                ...
                {
                    System.out.println("  step " + s.Duration + "s, start=" + s.StartPos.Name + " end = " + s.EndPos.Name + "; route = " + s.BusRoute);
                }
                System.out.println("----");
            }
            for (RouteEstimation estimate:estimations)
            {
                System.out.println(formatDate(estimate.ArrivalTime.getTime(), "MM/dd HH:mm"));
            }
        } catch (IOException e) {
    ...
    private static List<RouteEstimation> estimateRoutes(List<CandidateRoute> routes) throws IOException, RollbackException 
    {
            ...
            for (RouteStep step : route.Steps)
            {
                ...
                else if (step.Type.equals(StepType.Bus))
                {
                    ...
                    for (String dir : directions)
                        if (step.FullRouteName.toUpperCase().contains(dir.toUpperCase()))
                        {
                            direction = dir;
                            break;
                        }
                    ...
                    if (stops != null)
                    {
                        for (BusStop s : stops)
                        {
                            if (s.Name.toUpperCase().equals(step.StartPos.Name.toUpperCase()))
                                startStopId = s.Id;
                            if (s.Name.toUpperCase().equals(step.EndPos.Name.toUpperCase()))
                                endStopId = s.Id;
                        }
                    }
                    BusPrediction earliestBus = null;
                    
                    List<BusPrediction> predictions = getBusPrediction(step.BusRoute, startStopId);
                    ...
                    if (earliestBus != null && earliestBus.VehicleId != 0)
                    {
                        ...
                        Calendar parrivalTime = predictBusArrivalTime(step.BusRoute, earliestBus.VehicleId, endStopId);
    ...
    private static Calendar predictBusArrivalTime(String routeName, int vid, int stopId)
    {
        try
        {
            ...
            URL url = new URL(
                    "http://truetime.portauthority.org/bustime/api/v2/getpredictions?key=ADpCvpyDcupACyuMdk5wrVTVH&format=json&rt=" +
                            URLEncoder.encode(routeName, "UTF-8") + "&vid=" + vid);
            ...
            JSONArray predicts = (JSONArray) ((JSONObject) obj.get("bustime-response")).get("prd");
            for (Object predict : predicts)
            {
                int pstpid = Integer.parseInt(((JSONObject)predict).get("stpid").toString());
                if (pstpid == stopId)
                {
                    Calendar time = Calendar.getInstance();
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd HH:mm", Locale.US);
                    time.setTime(sdf.parse(((JSONObject)predict).get("prdtm").toString()));
                    return time;
                }
            }
            return null;
    ...
    private static List<BusPrediction> getBusPrediction(String routeName, int stopId) 
    {
        HttpURLConnection connection = null;
        URL url;
        try {
            url = new URL(
                    "http://truetime.portauthority.org/bustime/api/v2/getpredictions?key=ADpCvpyDcupACyuMdk5wrVTVH&format=json&rt=" +
                            URLEncoder.encode(routeName, "UTF-8") + "&stpid=" + stopId);
        ...
        try
        {
            ...
            for (Object predict : predicts)
            {
                ...
                SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd HH:mm", Locale.US);
                time.setTime(sdf.parse(((JSONObject)predict).get("prdtm").toString()));
                int vid = Integer.parseInt(((JSONObject)predict).get("vid").toString());
    ...
    private static List<String> getRouteDirections(String routeName) throws IOException, RollbackException 
    {
        HttpURLConnection connection = null;
        URL url = new URL(
        ...
        JSONObject obj = (JSONObject) JSONValue.parse(readResponse(connection));
        JSONArray directions = (JSONArray) ((JSONObject) obj.get("bustime-response")).get("directions");
    ...
    private static List<BusStop> getStops(String routeName, String dirName) throws IOException,
    RollbackException {
        HttpURLConnection connection = null;
        URL url = new URL(
                "http://truetime.portauthority.org/bustime/api/v2/getstops?key=ADpCvpyDcupACyuMdk5wrVTVH&format=json&rt=" +
                        URLEncoder.encode(routeName, "UTF-8")+"&dir=" + 
        ...
        JSONObject obj = (JSONObject) JSONValue.parse(readResponse(connection));
        JSONArray stops = (JSONArray) ((JSONObject) obj.get("bustime-response")).get("stops");
        List<BusStop> result = new ArrayList<BusStop>();
        for (Object stop : stops)
        {
            BusStop s = new BusStop();
            s.Id = Integer.parseInt(((JSONObject)stop).get("stpid").toString());
            s.Name = ((JSONObject)stop).get("stpnm").toString(); 
            s.Latitute = Double.parseDouble(((JSONObject)stop).get("lat").toString());
            s.Longitute = Double.parseDouble(((JSONObject)stop).get("lon").toString());
            result.add(s);
        }
        return result;
    }
    
    private static List<CandidateRoute> getAllRoutes(String origin, String dest) throws IOException,
            RollbackException {
        HttpURLConnection connection = null;
        URL url = new URL(
        ...
        for (Object route : routes)
        {
            ...
            for (Object leg : legs)
            {
                ...
                for (Object step : steps)
                {
                    ...
                    s.Duration = Integer.parseInt(((JSONObject)jstep.get("duration")).get("value").toString());
                    String travelMode = jstep.get("travel_mode").toString();
                    s.StartPos = new RouteLocation();
                    s.EndPos = new RouteLocation();
                    s.StartPos.Latitute = ((JSONObject)jstep.get("start_location")).get("lat").toString();
                    s.StartPos.Latitute = ((JSONObject)jstep.get("start_location")).get("lng").toString();
                    s.EndPos.Latitute = ((JSONObject)jstep.get("end_location")).get("lat").toString();
                    s.EndPos.Latitute = ((JSONObject)jstep.get("end_location")).get("lng").toString();
                    ...
                    {
                        s.Type = StepType.Bus;
                        JSONObject transitDetails = (JSONObject)jstep.get("transit_details");
                        s.BusRoute = ((JSONObject)transitDetails.get("line")).get("short_name").toString();
                        s.FullRouteName = ((JSONObject)transitDetails.get("line")).get("name").toString();
                        s.StartPos.Name = ((JSONObject)transitDetails.get("departure_stop")).get("name").toString();
                        s.EndPos.Name = ((JSONObject)transitDetails.get("arrival_stop")).get("name").toString();
                        parsedRoute.Steps.add(s);
                    }
                }
            }
